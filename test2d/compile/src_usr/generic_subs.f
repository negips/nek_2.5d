...
!     tensor3_op_ifi    : Tensor operations, user provided
!                         array and logicals for identity operation
...
!---------------------------------------------------------------------- 
      subroutine tensor3_op_ifi(fldf,fld,nx,ny,nz,opx,opyt,opzt
     $                          ,mx,my,mz,Ix,Iy,Iz,op_wk,ldwk)
      ...
      logical Ix  ! If opx  is Identity
      logical Iy  ! if opyt is Identity
      logical Iz  ! If opzt is Identity
...
!     y,z operators apply as transposed operators 
      ...
      integer ldwk
      real op_wk(ldwk**ldim,2)
!      common /tensor_op_work/ op_wk

      if ((mx*my*mz.gt.(ldwk**ldim)) .and. 
     $    (nx*ny*nz.gt.(ldwk**ldim))) then
        if (nio.eq.0) write(6,*) 
     $    'Inadequate workspace size in tensor3_op_ifi'
        call exitt
      endif  
      ...
      if (.not.Ix) then
        ...
        call copy(op_wk(1,1),fld,nx*ny*nz)
      endif
...
!       y-direction            
        if (.not.Iy) then
          ...
          call copy(op_wk(1,2),op_wk(1,1),mx*ny*nz)
        endif
...
!       z-direction
        if (.not.Iz) then    
          ...
          call copy(fldf,op_wk(1,2),mx*my*nz)
        endif       
...
!       y-direction            
        if (.not.Iy) then
          ...
          call copy(fldf,op_wk(1,1),mx*ny*nz)
        endif
